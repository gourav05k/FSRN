Creating everything in server.js is not advisable for complex projects.
For better preseentation of the appn, we will follow MVC architecture. (Model-View-Controller)

/**
 * 1. Create Database in MongoDB Atlas(cloud based database)
 * 2. Install mongoose 
 * 3. Connect server with mongodb 
 * 4. MVC
 * 5. Routes ---- path ("/api/restaurants" , controller)
 * 6. Controller ---- handle logic (post , fetch, update etc.)
 * 7. Model ---- (Database Table with schema)
 * 8. Use Mongoose Methods to work on this table (save() , find(), and so on)
 */